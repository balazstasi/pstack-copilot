...
- Copilot CLI: match a `copilot:*` descriptor to one Copilot-native family row, then dispatch `pstack-<stem>` on `task()` (`pstack:pstack-<stem>` only for `--agent`). The agent file pins `model` and the family's default `reasoning-effort`. Opus 5 also pins `context-tier: default`. Spawn-time `model` on `task()` is valid, but unused here. Do not pass spawn-time effort. `poteto-agent` and `comment-sicko` are not family lanes. Use an isolated worktree for a writer.
