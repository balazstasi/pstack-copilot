...
Transcripts live at `~/.copilot/logs` plus the current session, not Cursor transcript trees, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message.
