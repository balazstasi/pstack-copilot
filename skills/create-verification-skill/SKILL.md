...
**Platform note.** On Claude Code the generated skill lands under `.claude/skills/verify-<app>/`. On Copilot write it to `.github/skills/verify-<app>/`. On Codex or another runtime, use that runtime's project-skill location. The app-driving harness is platform-neutral. On Copilot, `view` `${PLUGIN_ROOT}/skills/poteto-mode/references/copilot-tools.md` as an absolute path. Do not `view` `../poteto-mode/references/copilot-tools.md` against the open workspace.
