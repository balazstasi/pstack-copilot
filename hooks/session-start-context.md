...
Before choosing any pstack workflow, resolve usage mode from the current Copilot model sheet (default ~/.copilot/pstack-models.md). Read the installed plugin's skills/poteto-mode/references/usage-mode.md once per session. In poor mode, all nested skills run locally without automatic agents or model panels. “I'm poor”, /poor-mode, and “normal mode” route to poor-mode; task-only overrides do not change saved preferences.
...
When the intent is already specific, enter directly: `/tdd` (bug with a reproducible failure), `/architect` (types and module shape; in poor mode only for meaningful boundary or ownership changes), `/how` (how a subsystem works), `/why` (why it was built this way), `/arena` (N parallel attempts at one task), `/interrogate` (multi-model diff review).
